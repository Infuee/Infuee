...
class Menu
{
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if ( ( isset($item['page']) && $item['page'] == '/'.Route::getFacadeRoot()->current()->uri() ) ||  (isset( $item['active'] )  && in_array(\Request::segment(2), $item['active']) )  ) {
            return true;
        }
    ...
    public static function isActiveHorMenuItem($item, $page, $rec = 0)
    {
        ...
        if (isset($item['page']) && $item['page'] == '/'.Route::getFacadeRoot()->current()->uri()) {
            // echo $item['page'].' '.'/'.Route::getFacadeRoot()->current()->uri();die;
            return true;
        }
